...
export default function AboutPage() {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Learn about the mission, history, and values of the Lebanese Association for Respiratory Care."
        />
      </Head>
  ...
  {/* Background Gradient with Soft Pattern */}
  <div className="bg-gradient-to-br from-blue-900 to-blue-700 text-white py-28 px-6 relative z-10">
    ...
    <div className="max-w-5xl mx-auto text-center relative z-10 animate-fade-in">
      ...
      <Link
        href="/membership"
        className="inline-block bg-white hover:bg-gray-100 text-blue-900 font-semibold px-6 py-3 rounded-full shadow-md transform hover:scale-105 transition-all duration-300"
      >
        Join Us
      </Link>
    </div>
    ...
    >
      ...
      />
        ...
        {[{
          ...
          <section
            key={index}
            className={`py-16 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'} px-6`}
          >
            <div className="max-w-4xl mx-auto">
              <h2 className="text-3xl font-bold text-gray-800 mb-6">{title}</h2>
              {content.split('\n').map((p, i) => (
                <p key={i} className="text-gray-700 leading-relaxed mb-6">
                  {p.split(/(Moustapha)/gi).map((part, j) =>
                    part.toLowerCase() === 'moustapha' ? (
                      <Link key={j} href="/founder" className="text-blue-800 font-semibold underline hover:text-blue-600">Moustapha</Link>
                    ) : (
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="relative border-l-4 border-blue-600 ml-4">
              {[{
                ...
                desc: `Moustapha launched the first academic respiratory therapy program at Makassed University of Beirut, in partnership with ICRC and the American Association for Respiratory Care (AARC).`
              }, {
                year: '2024',
                ...
                <div key={i} className="mb-10 ml-4 md:ml-8 relative">
                  ...
                  <p className="mt-2 text-gray-700">
                    {item.desc.split(/(Moustapha)/gi).map((part, j) =>
                      part.toLowerCase() === 'moustapha' ? (
                        <Link key={j} href="/founder" className="text-blue-800 font-semibold underline hover:text-blue-600">Moustapha</Link>
                      ) : (
                        <span key={j}>{part}</span>
        ...
        {[{
          ...
          <section
            key={i}
            className={`py-16 px-6 ${section.bg}`}
          >
            <div className="max-w-4xl mx-auto">
